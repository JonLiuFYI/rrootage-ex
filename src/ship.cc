...
void moveShip() {
  ...
  int joymove = (padst.x != 0 || padst.y != 0);
  

  if ( sd >= 0 || joymove) {

      //joystick motion vs button motion
      if (joymove)
      {
          ship.pos.x += (ship.speed * padst.x);
          ship.pos.y += (ship.speed * padst.y);
      }
      else {
		  ship.pos.x += ((ship.speed * shipMv[sd][0]) >> 8);  // * 0.3;
		  ship.pos.y += ((ship.speed * shipMv[sd][1]) >> 8);  // * 0.3;
      }
